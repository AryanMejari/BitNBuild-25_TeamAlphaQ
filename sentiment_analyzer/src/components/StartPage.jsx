...
const StartPage = ({ onAnalyzeClick, currentUrl }) => {
  ...
  <img src={logo} className="w-20 h-20 object-contain rounded-full" alt="Review Radar Logo" />
</div>
    ...
    {currentUrl && (
      <div className="mb-8 p-4 bg-gray-700/50 rounded-lg border border-gray-600 shadow-sm backdrop-blur-sm">
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="text-sm font-medium text-white">{getDomain(currentUrl)}</span>
        </div>
      </div>
    )}
    ...
    <div className="text-center mb-8">
      ...
      <p className="text-gray-300 text-sm leading-relaxed mb-6">
        Get instant insights from customer reviews with AI-powered sentiment analysis and keyword extraction.
      </p>
    </div>
...
>
  <span className="relative z-10 tracking-wider">Generate Review Analysis</span>
